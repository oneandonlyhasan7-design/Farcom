...
class ThirdPartySipAccountLoginViewModel
    ...
    companion object {
        ...
        const val MODE_SELF_HOSTED = "self_hosted"
    }
    ...
    // Tracks which connection mode the user picked ("central" or "self_hosted") purely to
    // drive the UI (highlighting the selected button); it does not gate what the user can
    // type, they can still freely edit every field afterwards.
    val serverMode = MutableLiveData<String>()

    // True when a central server domain has been configured by the deployer (via
    // linphonerc_default). If false, the "Central server" quick-fill button still works
    // but leaves the domain field empty for the user to fill in themselves, since there's
    // no central server to point at.
    val centralServerConfigured = MutableLiveData<Boolean>()
    ...
    init {
        ...
        coreContext.postOnCoreThread {
            centralServerConfigured.postValue(corePreferences.farcomCentralServerDomain.isNotEmpty())

            val lastMode = corePreferences.farcomLastSelectedServerMode
            if (lastMode == MODE_CENTRAL && corePreferences.farcomCentralServerDomain.isNotEmpty()) {
                serverMode.postValue(MODE_CENTRAL)
                applyCentralServerDefaults()
            } else if (lastMode == MODE_SELF_HOSTED) {
                serverMode.postValue(MODE_SELF_HOSTED)
                applySelfHostedDefaults()
            } else {
    ...
    // Pre-fills the form to connect to the deployer's central Farcom server, if one has
    // been configured (see CorePreferences.farcomCentralServerDomain). Leaves username/
    // password untouched so the user just has to fill in their credentials.
    @UiThread
    fun useCentralServer() {
        serverMode.value = MODE_CENTRAL
        coreContext.postOnCoreThread {
            applyCentralServerDefaults()
            corePreferences.farcomLastSelectedServerMode = MODE_CENTRAL
        }
    }

    // Pre-fills the form to connect directly to a self-hosted/LAN SIP server, defaulting
    // to 127.0.0.1 (editable) with no dependency on any central server being reachable.
    @UiThread
    fun useSelfHostedServer() {
        serverMode.value = MODE_SELF_HOSTED
        coreContext.postOnCoreThread {
            applySelfHostedDefaults()
            corePreferences.farcomLastSelectedServerMode = MODE_SELF_HOSTED
        }
    }

    @WorkerThread
    private fun applyCentralServerDefaults() {
        val centralDomain = corePreferences.farcomCentralServerDomain
        domain.postValue(centralDomain)
        val transportName = corePreferences.farcomCentralServerDefaultTransport.uppercase(Locale.getDefault())
        val index = availableTransports.indexOf(transportName).takeIf { it >= 0 }
            ?: availableTransports.size - 1
        defaultTransportIndexEvent.postValue(Event(index))
    }

    @WorkerThread
    private fun applySelfHostedDefaults() {
        // Only overwrite the domain field if it's empty or still the central domain, so we
        // don't clobber an IP/hostname the user already typed in.
        val current = domain.value.orEmpty().trim()
        val central = corePreferences.farcomCentralServerDomain
        if (current.isEmpty() || current == central) {
            domain.postValue(corePreferences.farcomSelfHostedDefaultAddress)
        }
        val transportName = corePreferences.farcomSelfHostedDefaultTransport.uppercase(Locale.getDefault())
        val index = availableTransports.indexOf(transportName).takeIf { it >= 0 } ?: 0
        defaultTransportIndexEvent.postValue(Event(index))
    }
