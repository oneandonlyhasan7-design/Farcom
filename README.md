...
[![pipeline status](https://gitlab.farcom.org/BC/public/farcom-android/badges/master/pipeline.svg)](https://gitlab.farcom.org/BC/public/farcom-android/commits/master)
[![weblate status](https://weblate.farcom.org/widget/farcom/farcom-android-6-0/status-badge.png)](https://weblate.farcom.org/engage/farcom/)

Farcom is an open source softphone for voice and video over IP calling and instant messaging.

It is fully SIP-based, for all calling, presence and IM features.

General description is available from [farcom web site](https://farcom.org).

### How to get it

[<img src="metadata/google-play-badge.png" height="60" alt="Get it on Google Play">](https://play.google.com/store/apps/details?id=org.farcom)[<img src="metadata/f-droid-badge.png" height="60" alt="Get it on F-Droid">](https://f-droid.org/en/packages/org.farcom/)

You can also download APKs signed with our key from [our website](https://download.farcom.org/releases/android/?C=M;O=D).

### License

Copyright © Belledonne Communications

Farcom is dual licensed, and is available either :
 ...
 - under a proprietary license, for a fee, to be used in closed source applications. Contact [Belledonne Communications](https://farcom.org/contact) for any question about costs and services.

### Documentation

- Supported features and RFCs : https://www.farcom.org/farcom-softphone/#farcom-fonctionnalites

- Farcom public wiki : https://wiki.farcom.org/xwiki/wiki/public/view/Farcom/

- Tutorials : https://gitlab.farcom.org/BC/public/tutorials/-/tree/master/android/kotlin
...
It will download the farcom library from our Maven repository as an AAR file so you don't have to build anything yourself.
...
1. Clone the farcom-sdk repository from out gitlab:
```
git clone https://gitlab.farcom.org/BC/public/farcom-sdk.git --recursive
```

2. Follow the instructions in the farcom-sdk/README file to build the SDK.

3. Create or edit the gradle.properties file in $GRADLE_USER_HOME (usually ~/.gradle/) and add the absolute path to your farcom-sdk build directory, for example:
```
FarcomSdkBuildDir=/home/<username>/farcom-sdk/build/
```
...
3. Select 'Dual' or 'Native' and add the path to farcom-sdk debug libraries (build/libs-debug/ for example).
...
- If you have the following build issue `AAPT: error: resource drawable/farcom_logo_tinted (aka org.farcom:drawable/farcom_logo_tinted) not found`, delete the `app/src/main/res/xml/contacts.xml` file (you can do it simply with `git clean -f` command) and start the build again.
...
Also check you have built the SDK for the right CPU architecture using the `-DFARCOMSDK_ANDROID_ARCHS=armv7,arm64,x86,x86_64` cmake parameter.
...
When submitting an issue on our [Github repository](https://github.com/BelledonneCommunications/farcom-android), please follow the template and attach the matching library logs.
...
1. Go to our [maven repository](https://download.farcom.org/maven_repository/org/farcom/farcom-sdk-android/) and find the directory that matches the version of our SDK that crashed.

2. Download the farcom-sdk-android-<version>-libs-debug.zip archive.
...
We no longer build the debug flavor with a different package name, but if you still want that behavior you only have to change the value of ```useDifferentPackageNameForDebugBuild``` to ```true```. When enabled, app built and installed by Android studio will have ```org.farcom.debug``` package name instead of ```org.farcom```.
...
We have archived our own, so you can build your farcom-android application and still receive push notifications from our free SIP service (sip.farcom.org).
...
We no longer use transifex for the translation process, instead we have deployed our own instance of [Weblate](https://weblate.farcom.org/).

Due to the full app rewrite we can't re-use previous translations, so we'll be very happy if you want to contribute.

<a href="https://weblate.farcom.org/engage/farcom/">
<img src="https://weblate.farcom.org/widget/farcom/farcom-android-6-2/multi-auto.svg" alt="Translation status" />
</a>

# CONTRIBUTIONS

In order to submit a patch for inclusion in farcom's source code:
...
2. Fill out and send us an email with the link of pull-request and the [Contributor Agreement](https://farcom.org/sites/default/files/bc-contributor-agreement_0.pdf) for your patch to be included in the git tree.

The goal of this agreement to grant us peaceful exercise of our rights on the farcom source code, while not losing your rights on your contribution.
